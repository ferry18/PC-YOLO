...
class C2(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5):
        ...
        """
        super().__init__()
        self.c = max(1, int(c2 * e))  # hidden channels (ensure minimum of 1)
...
# Custom modules for PC-YOLO architecture
class Bottleneck_WDBB(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        from .conv import RepConv
        # Using RepConv as a temporary replacement for WideDiverseBranchBlock
        self.cv1 = RepConv(c1, c_, k[0], 1)
        self.cv2 = RepConv(c_, c2, k[1], 1, g=g)
...
class PDB(nn.Module):
    def __init__(self, c1):
        """
        Initialize PDB (Parallel Decomposition Block) module.
        ...
        self.ka = nn.Sequential(
            nn.Conv2d(c1, c1 // 8, 1, padding=0, bias=True),
            nn.GELU(),
            nn.Conv2d(c1 // 8, c1, 1, padding=0, bias=True),
            nn.Sigmoid()
        )

        # 空间注意力
        self.td = nn.Sequential(
            default_conv(c1, c1, 1),
            default_conv(c1, c1 // 8, 3),
            nn.GELU(),
            default_conv(c1 // 8, c1, 3),
            nn.Sigmoid()
        )
...
class CALayer(nn.Module):
    def __init__(self, c1, c2=None):
        ...
        self.ca = nn.Sequential(
            nn.Conv2d(c1, c2 // 8, 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(c2 // 8, c2, 1, padding=0, bias=True),
            nn.Sigmoid()
        )
...
class PALayer(nn.Module):
    def __init__(self, c1, c2=None):
        """
        Initialize Position Attention Layer.
        ...
        self.pa = nn.Sequential(
            nn.Conv2d(c1, c2 // 8, 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(c2 // 8, 1, 1, padding=0, bias=True),
            nn.Sigmoid()
        )
...
class PFEB(nn.Module):
    def __init__(self, c1, c2=None):
        """
        Initialize PFEB (Parallel Feature Enhancement Block) module.
        ...
        # If input and output channels are different, add a projection layer
        self.project = None
        if c1 != c2:
            self.project = Conv(c1, c2, 1, 1)
            
        # Use c2 for attention layers since we want output to have c2 channels
        self.ca = CALayer(c2, c2)
        self.pa = PALayer(c2, c2)
        self.pdb = PDB(c2)  # 将PDB模块加入PFEB

    def forward(self, x):
        # Project to output channel size if needed
